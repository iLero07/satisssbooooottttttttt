...
// discord.gg/altyapilar - Lourity
    ...
    if (interaction.customId === 'productCancel') {
        const productCancelEmbed = new EmbedBuilder()
            .setColor("Red")
            .setDescription(`${interaction.user} siparişin iptal edilmesini sipariş etti, \`10 saniye\` sonra edilecektir.`)
    ...
    if (interaction.customId === "ticketEnd") {
        ...
        client.channels.cache.get(config.log).send({ embeds: [log], components: [message_button] }).then((message) => {
            ...
            if (ticket_messages) {
                ...
                if (messages) {
                    fs.writeFileSync(`./productMessages/${message.id}-sipariş.txt`, messages)
                }
            }
        })
        ...
        const deleted_channel = new EmbedBuilder()
            .setColor("Red")
            .setDescription(`sipariş **5 saniye** içerisinde silinecektir. (${interaction.user.username} tarafından)`)
    ...
    if (interaction.customId === "ticket_message") {
        if (fs.existsSync(`./productMessages/${interaction.message.id}-sipariş.txt`)) {
            interaction.reply({ files: [`./productMessages/${interaction.message.id}-sipariş.txt`], ephemeral: true })
        } else {
            const message_button = new ActionRowBuilder()
                .addComponents(
                    new ButtonBuilder()
